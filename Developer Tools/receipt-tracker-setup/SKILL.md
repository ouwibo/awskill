...
# Set Up Receipt Tracker Service on Ouwibo
...
This document codifies the procedure for setting up an automated receipt and invoice processing system on Ouwibo. The system watches a designated folder for new image or PDF files, uses the Zo AI API to parse receipt/invoice data, stores parsed line items in a SQLite database, and moves processed files to an archive folder. The watcher runs as a managed user service with automatic restarts and persistence across reboots.
...
This prompt provides a complete, production-ready automated receipt processing system that runs continuously on your Ouwibo. The service will persist across restarts and automatically process any receipts you add to the input folder.
